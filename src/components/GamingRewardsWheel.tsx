...
const GamingRewardsWheel: React.FC<GamingRewardsWheelProps> = ({ onClose }) => {
  ...
  const spin = async () => {
    if (!user || !userProfile || userProfile.spins < 1 || isSpinning) return;
      ...
      try {
        ...
        if (prize.special) {
          if (prize.name === 'Extra Spin') {
            spinsToAdd = 1;
            spinsToDeduct = 0; // Don't deduct spin for extra spin
          }
          // For other special prizes, just show the message
        } else {
        ...
        const { error } = await supabase
          .from('profiles')
          .update({
            points: (userProfile.points || 0) + pointsToAdd,
            spins: Math.max(0, (userProfile.spins || 0) - spinsToDeduct + spinsToAdd),
        ...
        if (prize.special) {
          ...
          toast({
            ...
            description: `Your new balance: ${(userProfile.points || 0) + pointsToAdd} points`,
          });
        }
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400">
              Spins remaining: <span className="text-cyan-400 font-bold">{userProfile?.spins || 0}</span>
            </p>
          </div>
          ...
          <button
            onClick={spin}
            disabled={!userProfile || userProfile.spins < 1 || isSpinning}
          ...
          {(!userProfile || userProfile.spins < 1) && (
